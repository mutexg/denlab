...
#if defined(AK_OS_DENLAB) && !defined(KERNEL)
#    include <denlab.h>
#endif
...
// On DenLab, dbgln goes to a non-stderr output
...
void vdbg(StringView fmtstr, TypeErasedFormatParams& params, bool newline)
{
    ...
    if (is_rich_debug_enabled) {
        ...
        if (!process_name.is_empty()) {
            ...
            auto pid = getpid();
#    if defined(AK_OS_DENLAB) || defined(AK_OS_LINUX)
            // Linux and DenLab handle thread IDs as if they are related to process ids
            auto tid = gettid();
            if (pid == tid)
#    endif
            {
