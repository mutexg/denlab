...
DenLab-specific patches were upstreamed to CMake in major version 3.25. To avoid carrying
patches to CMake, the minimum required CMake to build DenLab is set to that version.
...
If you have previously compiled DenLabOS with an older or distribution-provided version of CMake,
...
If you're on Windows you can use WSL2 to build DenLabOS. Please have a look at the [Windows guide](BuildInstructionsWindows.md)
for details.
...
Optional: `fuse2fs` for [building images without root](https://github.com/DenLabOS/denlab/pull/11224), and `clang llvm llvm-libs` for building with Clang.
...
Run the following command to build and run DenLabOS:

```console
Meta/denlab.sh run
```

This will compile all of DenLabOS and install the built files into the `Build/<architecture>/Root` directory inside your Git
repository. It will also build a disk image and start DenLabOS using QEMU. The chosen architecture defaults to
...
the DenLabOS cross-compiler toolchain. These steps only have to be done once, so the next build will go much faster.
...
`Meta/denlab.sh build`. The `denlab.sh` script also provides a number of other commands. Run the script without
arguments for a list.

## Ports

To add a package from the ports collection to DenLab, for example curl, change into the `Ports/curl` directory and
run `./package.sh`. The source code for the package will be downloaded and the package will be built. The next time you
start DenLab, `curl` will be available.
...
At this point you should have a fully functioning VM for DenLabOS. The [advanced build instructions guide](AdvancedBuildInstructions.md)
