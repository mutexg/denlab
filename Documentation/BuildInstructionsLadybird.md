...
The simplest way to build and run ladybird is via the denlab.sh script:

```bash
# From /path/to/denlab
./Meta/denlab.sh run lagom ladybird
./Meta/denlab.sh gdb lagom ladybird
```
...
directory to always build LibWeb and Ladybird for Lagom when rebuilding DenLabOS using the
denlab.sh script to run a qemu instance.

To restore the previous behavior that only builds code generators and tools from Lagom when
rebuilding denlab, you must modify the CMake cache back to the default.
...
Ladybird requires resource files from the denlab/Base/res directory in order to properly load
icons, fonts, and other theming information. The denlab.sh script calls into custom CMake targets
...
Note that when using a custom build directory rather than Meta/denlab.sh, the user may need to provide
...
After running Ladybird as suggested above with `./Meta/denlab.sh run lagom ladybird`, you can now in CLion use Run -> Attach to Process to connect. If debugging layouting and rendering issues, filter the listing that opens for `WebContent` and attach to that.

Now breakpoints, stepping and variable inspection will work.

### Debugging with Xcode on macOS

The `denlab.sh` build script does not know how to generate Xcode projects, so creating the project must be done manually.
To be compatible with the `denlab.sh` script, a few extra options are required. If there is a previous Lagom build directory, CMake will likely complain that the generator has changed.

```
cmake -GXcode -S Meta/Lagom -B Build/lagom -DBUILD_LAGOM=ON -DENABLE_LAGOM_LADYBIRD=ON
```

Alternatively, if you don't need your ladybird build to be compatible with `denlab.sh`, you can use Ladybird as the source directory like so:
