...
DenLabOS can be built and run under WSL Version 2.
...
Next, go to [BuildInstructions.md](https://github.com/DenLabOS/denlab/blob/master/Documentation/BuildInstructions.md#prerequisites)
...
In practice, this means cloning and building the project to somewhere such as `/home/username/denlab`. You can then
access the linux filesystem at `\\wsl$`, so for example, the project would be at `\\wsl$\home\username\denlab`.
...
Run `Meta/denlab.sh run` to build and run DenLabOS as usual.
...
Afterwards you can start the VM with `Meta/denlab.sh run` as usual.
