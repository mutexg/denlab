...
the DenLab code base. This document aims to track and describe them, so they
can be propagated further and the code base can be kept consistent.

## `TRY(...)` Error Handling

The `TRY(...)` macro is used for error propagation in the DenLab code base.
...
The usual C++ constructors are incompatible with DenLabOS's method of handling errors,
...
## The `denlab_main(...)` program entry point

DenLab has moved to a pattern where executables do not expose a normal C
main function. A `denlab_main(...)` is exposed instead. The main reasoning
is that the `Main::Arguments` struct can provide arguments in a more idiomatic
way that fits with the DenLab API surface area. The `ErrorOr<int>` likewise
...
`denlab_main(...)` on program startup.

The creation of the pattern was documented in the following video:
[OS hacking: A better main() for DenLabOS C++ programs](https://www.youtube.com/watch?v=5PciKJW1rUc).
...
Instead, `denlab_main(..)` is defined like this:
...
are used in the DenLabOS userland. A data structure is said to be
...
DenLabOS. It's become the idiomatic way to capture the location
