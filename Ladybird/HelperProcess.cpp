...
ErrorOr<NonnullRefPtr<Protocol::RequestClient>> launch_request_server_process(ReadonlySpan<ByteString> candidate_request_server_paths, StringView denlab_resource_root, Vector<ByteString> const& certificates)
{
    Vector<ByteString> arguments;

    if (!denlab_resource_root.is_empty()) {
        arguments.append("--denlab-resource-root"sv);
        arguments.append(denlab_resource_root);
    }
