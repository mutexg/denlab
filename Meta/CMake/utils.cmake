...
include(${CMAKE_CURRENT_LIST_DIR}/denlab_components.cmake)
include(${CMAKE_CURRENT_LIST_DIR}/code_generators.cmake)

find_package(Python3 COMPONENTS Interpreter REQUIRED)
include(GNUInstallDirs)

function(denlab_set_implicit_links target_name)
...
function(denlab_install_headers target_name)
...
function(denlab_install_sources)
    ...
    foreach(source ${sources})
        ...
        install(FILES ${source} DESTINATION usr/src/denlab/${current_source_dir_relative}/${subdirectory} OPTIONAL)
    endforeach()
endfunction()

function(denlab_generated_sources target_name)
...
if (NOT COMMAND denlab_lib)
    function(denlab_lib target_name fs_name)
        cmake_parse_arguments(PARSE_ARGV 2 DENLAB_LIB "" "TYPE" "")

        if ("${DENLAB_LIB_TYPE}" STREQUAL "")
            set(DENLAB_LIB_TYPE SHARED)
        endif()

        denlab_install_headers(${target_name})
        denlab_install_sources()
        add_library(${target_name} ${DENLAB_LIB_TYPE} ${SOURCES} ${GENERATED_SOURCES})
        set_target_properties(${target_name} PROPERTIES EXCLUDE_FROM_ALL TRUE)
        set_target_properties(${target_name} PROPERTIES VERSION "denlab")
...
function(denlab_libc target_name fs_name)
    denlab_install_headers("")
    denlab_install_sources()
...
if (NOT COMMAND denlab_add_executable)
    function(denlab_add_executable name)
        # if any of the sources are .jakt files, switch to using denlab_jakt_executable()
        ...
        if (jakt_source)
            if (ENABLE_JAKT)
                denlab_jakt_executable(${name} MAIN_SOURCE ${jakt_source} CPP_SOURCES ${cpp_sources})
            else()
                ...
                set_target_properties(${name} PROPERTIES DENLAB_SKIP_BUILD TRUE)
            endif()
        else()
...
if (NOT COMMAND denlab_bin)
    function(denlab_bin target_name)
        denlab_install_sources()
        denlab_add_executable(${target_name} ${SOURCES})
        target_link_libraries(${target_name} PUBLIC GenericClangPlugin)
        set_target_properties(${target_name} PROPERTIES EXCLUDE_FROM_ALL TRUE)
...
if (NOT COMMAND denlab_test)
    function(denlab_test test_src sub_dir)
        cmake_parse_arguments(PARSE_ARGV 2 DENLAB_TEST "MAIN_ALREADY_DEFINED" "CUSTOM_MAIN" "LIBS")
        set(TEST_SOURCES ${test_src})
        if ("${DENLAB_TEST_CUSTOM_MAIN}" STREQUAL "")
            set(DENLAB_TEST_CUSTOM_MAIN "$<TARGET_OBJECTS:LibTestMain>")
        endif()
        if (NOT ${DENLAB_TEST_MAIN_ALREADY_DEFINED})
            list(PREPEND TEST_SOURCES "${DENLAB_TEST_CUSTOM_MAIN}")
        endif()
        get_filename_component(test_name ${test_src} NAME_WE)
        add_executable(${test_name} ${TEST_SOURCES})
        add_dependencies(ComponentTests ${test_name})
        set_target_properties(${test_name} PROPERTIES EXCLUDE_FROM_ALL TRUE)
        denlab_set_implicit_links(${test_name})
        target_link_libraries(${test_name} PRIVATE LibTest LibCore LibFileSystem)
        foreach(lib ${DENLAB_TEST_LIBS})
...
function(denlab_testjs_test test_src sub_dir)
    cmake_parse_arguments(PARSE_ARGV 2 DENLAB_TEST "" "CUSTOM_MAIN" "LIBS")
    if ("${DENLAB_TEST_CUSTOM_MAIN}" STREQUAL "")
        set(DENLAB_TEST_CUSTOM_MAIN "$<TARGET_OBJECTS:JavaScriptTestRunnerMain>")
    endif()
    list(APPEND DENLAB_TEST_LIBS LibJS LibCore LibFileSystem)
    denlab_test(${test_src} ${sub_dir}
        CUSTOM_MAIN "${DENLAB_TEST_CUSTOM_MAIN}"
        LIBS ${DENLAB_TEST_LIBS})
endfunction()

function(denlab_app target_name)
    cmake_parse_arguments(PARSE_ARGV 1 DENLAB_APP "" "ICON" "")

    set(small_icon "${DenLabOS_SOURCE_DIR}/Base/res/icons/16x16/${DENLAB_APP_ICON}.png")
    set(medium_icon "${DenLabOS_SOURCE_DIR}/Base/res/icons/32x32/${DENLAB_APP_ICON}.png")

    denlab_bin("${target_name}")

    if (EXISTS "${small_icon}")
        embed_resource("${target_name}" denlab_icon_s "${small_icon}")
    else()
        message(FATAL_ERROR "Missing small app icon: ${small_icon}")
    endif()

    if (EXISTS "${medium_icon}")
        embed_resource("${target_name}" denlab_icon_m "${medium_icon}")
    else()
        ...
        if (NOT ${DENLAB_APP_ICON} IN_LIST allowed_missing_medium_icons)
...
function(embed_resource target section file)
    ...
    add_custom_command(
        OUTPUT "${asm_file}"
        COMMAND "${DenLabOS_SOURCE_DIR}/Meta/generate-embedded-resource-assembly.sh" "${asm_file}" "${section}" "${input_file}" "${file_size}"
        DEPENDS "${input_file}" "${DenLabOS_SOURCE_DIR}/Meta/generate-embedded-resource-assembly.sh"
...
function(download_file_multisource urls path)
    ...
    foreach(url ${urls})
        set(DOWNLOAD_COMMAND "${Python3_EXECUTABLE}" "${DenLabOS_SOURCE_DIR}/Meta/download_file.py" "-o" "${path}" "${url}")
