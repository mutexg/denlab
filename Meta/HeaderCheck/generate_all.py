...
if __name__ == '__main__':
    if 'DENLAB_SOURCE_DIR' not in os.environ:
        print('Must set DENLAB_SOURCE_DIR first!', file=sys.stderr)
        exit(1)
    if len(sys.argv) == 2:
        run(os.environ['DENLAB_SOURCE_DIR'], sys.argv[1])
    else:
        print('Usage: DENLAB_SOURCE_DIR=/path/to/denlab {} DENLAB_BUILD_ARCH'
