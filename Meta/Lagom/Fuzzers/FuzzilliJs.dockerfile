...
FROM fedora:39 AS denlab-build

WORKDIR /home
RUN dnf install -y clang cmake git-core ninja-build
RUN git clone --depth=1 https://github.com/DenLabOS/denlab

RUN cd denlab/Meta/Lagom && ./BuildFuzzers.sh
...
COPY --from=denlab-build /home/denlab/Meta/Lagom/Build/lagom-fuzzers/bin ./bin
COPY --from=denlab-build /home/denlab/Meta/Lagom/Build/lagom-fuzzers/lib64 ./lib64
...
CMD [ "sh", "-c", "./FuzzilliCli --profile=denlab --storagePath=fuzzilli-storage ${FUZZILLI_CLI_OPTIONS} ./bin/FuzzilliJs" ]
