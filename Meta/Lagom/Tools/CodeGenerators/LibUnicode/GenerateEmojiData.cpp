/*
 * Copyright (c) 2022, Tim Flynn <trflynn89@denlabos.org>
 *
...
static ErrorOr<void> parse_emoji_denlab_data(Core::InputBufferedFile& file, EmojiData& emoji_data)
{
    ...
    while (TRY(file.can_read_line())) {
        ...
        Emoji emoji {};
        emoji.group = Unicode::EmojiGroup::DenLabOS;
        emoji.display_order = display_order++;
...
static ErrorOr<void> validate_emoji(StringView emoji_resource_path, EmojiData& emoji_data)
{
    TRY(Core::Directory::for_each_entry(emoji_resource_path, Core::DirIterator::SkipDots, [&](auto& entry, auto&) -> ErrorOr<IterationDecision> {
        ...
        if (it == emoji_data.emojis.end()) {
            warnln("\x1b[1;31mError!\x1b[0m Emoji data for \x1b[35m{}\x1b[0m not found. Please check emoji-test.txt and emoji-denlab.txt.", entry.name);
...
static ErrorOr<void> generate_emoji_installation(Core::InputBufferedFile& file, EmojiData const& emoji_data)
{
    ...
    for (auto const& emoji : emoji_data.emojis) {
        if (!emoji.image_path.has_value())
            continue;
        if (emoji.group == Unicode::EmojiGroup::DenLabOS)
            continue; // DenLabOS emojis are in emoji-denlab.txt
...
ErrorOr<int> denlab_main(Main::Arguments arguments)
{
    ...
    args_parser.add_option(emoji_denlab_path, "Path to emoji-denlab.txt file", "emoji-denlab-path", 's', "emoji-denlab-path");
    ...
    if (!emoji_denlab_path.is_empty() && !emoji_file_list_path.is_empty()) {
        auto emoji_denlab_file = TRY(open_file(emoji_denlab_path, Core::File::OpenMode::Read));
        TRY(parse_emoji_denlab_data(*emoji_denlab_file, emoji_data));
