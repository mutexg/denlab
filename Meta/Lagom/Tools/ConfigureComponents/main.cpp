...
int main()
{
    ...
    auto configs_result = run_whiptail(WhiptailMode::Menu, configs, "DenLabOS - System Configurations"sv, "Which system configuration do you want to use or customize?"sv);
    ...
    if (customize) {
        ...
        auto result = run_whiptail(WhiptailMode::Checklist, options, "DenLabOS - System Components"sv, "Which optional system components do you want to include?"sv);
        if (result.is_error()) {
            warnln("ConfigureComponents cancelled.");
            return 0;
        }
