...
DISK_SIZE=$(($(disk_usage "$DENLAB_SOURCE_DIR/Base") + $(disk_usage Root) + 512 + 300))
...
parted -s "${dev}" mklabel gpt mkpart EFI fat32 0% 512MB mkpart DenLabOS ext2 512MB 100% set 1 esp on || die "couldn't partition disk"
echo "done"

printf "creating new filesystems... "
mkfs.vfat -F 32 -n EFI "${dev}p1" >/dev/null || die "couldn't create EFI system partition filesystem"
mke2fs -q -L DenLabOS "${dev}p2" || die "couldn't create root filesystem"
echo "done"
...
menuentry 'DenLabOS (normal)' {
  chainloader /Kernel.efi root=PARTUUID:${root_uuid}
}

menuentry 'DenLabOS (text mode)' {
  chainloader /Kernel.efi graphics_subsystem_mode=off root=PARTUUID:${root_uuid}
}

menuentry 'DenLabOS (No ACPI)' {
  chainloader /Kernel.efi root=PARTUUID:${root_uuid} acpi=off
}

menuentry 'DenLabOS (with serial debug)' {
  chainloader /Kernel.efi serial_debug root=PARTUUID:${root_uuid}
}
EOF

# FIXME: Mount the EFI system partition on /boot (both here and in denlab), so we don't need to move the Kernel.efi file to the ESP.
