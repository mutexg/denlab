...
INODE_SIZE=256
INODE_COUNT=$(($(inode_usage "$DENLAB_SOURCE_DIR/Base") + $(inode_usage Root)))
INODE_COUNT=$((INODE_COUNT + 2000))  # Some additional inodes for toolchain files, could probably also be calculated
DISK_SIZE_BYTES=$((($(disk_usage "$DENLAB_SOURCE_DIR/Base") + $(disk_usage Root) ) * 1024 * 1024))
DISK_SIZE_BYTES=$((DISK_SIZE_BYTES + (INODE_COUNT * INODE_SIZE)))

if [ -z "$DENLAB_DISK_SIZE_BYTES" ]; then
    ...
    if [ "$DISK_SIZE_BYTES" -gt "$DENLAB_DISK_SIZE_BYTES" ]; then
        die "DENLAB_DISK_SIZE_BYTES is set to $DENLAB_DISK_SIZE_BYTES but required disk size is $DISK_SIZE_BYTES bytes"
    fi
    DISK_SIZE_BYTES="$DENLAB_DISK_SIZE_BYTES"
fi

if [ -n "$DENLAB_INODE_COUNT" ]; then
    if [ "$INODE_COUNT" -gt "$DENLAB_INODE_COUNT" ]; then
        die "DENLAB_INODE_COUNT is set to $DENLAB_INODE_COUNT but required inode count is roughly $INODE_COUNT"
    fi
    INODE_COUNT="$DENLAB_INODE_COUNT"
fi
...
if [ "$DENLAB_ARCH" = "aarch64" ] || [ "$DENLAB_BOOT_DRIVE" = "pci-sd" ]; then
