...
for md_file in $(find "${MAN_DIR}" -iname '*.md' | ${SORT}); do
    ...
    pandoc -f gfm -t html5 -s \
        -B Meta/Websites/man.denlabos.org/banner-preamble.inc \
        --lua-filter=Meta/convert-markdown-links.lua \
        --lua-filter=Meta/Websites/man.denlabos.org/add-anchors.lua \
        --metadata title="${name}(${section_number}) - DenLabOS man pages" \
        -o "${output_file}" \
        "${md_file}" &
done
...
for section_directory in output/*/; do
    ...
    case "${section_number}" in
        ...
        *) section_title="DenLabOS man pages"; echo "WARNING: Unrecognized section ${section_number}?!";;
    esac
...
pandoc -f gfm -t html5 -s \
    -B Meta/Websites/man.denlabos.org/banner-preamble.inc \
    --metadata title="DenLabOS man pages" \
    -o output/index.html \
    Meta/Websites/man.denlabos.org/index.md &
pandoc -f gfm -t html5 -s \
    -B Meta/Websites/man.denlabos.org/banner-preamble.inc \
    --metadata title="Can't run applications" \
    -o output/cant-run-application.html \
    Meta/Websites/man.denlabos.org/cant-run-application.md &

# Copy pre-made files
echo 'Copying images'
rsync -a Meta/Websites/man.denlabos.org/banner.png output/ &
