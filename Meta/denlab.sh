...
Usage: $NAME COMMAND [TARGET] [TOOLCHAIN] [ARGS...]
  Supported TARGETs: aarch64, x86_64, riscv64, lagom. Defaults to DENLAB_ARCH, or host architecture if not set.
  Supported TOOLCHAINs: GNU, Clang. Defaults to DENLAB_TOOLCHAIN, or GNU if not set.
  Supported COMMANDs:
    ...
    copy-src:   Same as image, but also copies the project's source tree to ~/Source/denlab
...
exit_if_running_as_root "Do not run denlab.sh as root, your Build directory will become root-owned"
...
if [ "$TARGET" != "lagom" ] && [ -n "$1" ]; then
    ...
    TOOLCHAIN_TYPE="${DENLAB_TOOLCHAIN:-"GNU"}"
fi
if ! [[ "${TOOLCHAIN_TYPE}" =~ ^(GNU|Clang)$ ]]; then
    >&2 echo "ERROR: unknown toolchain '${TOOLCHAIN_TYPE}'."
    exit 1
fi
CMAKE_ARGS+=( "-DDENLAB_TOOLCHAIN=$TOOLCHAIN_TYPE" )
...
is_valid_target() {
    if [ "$TARGET" = "aarch64" ]; then
        CMAKE_ARGS+=("-DDENLAB_ARCH=aarch64")
        return 0
    fi
    if [ "$TARGET" = "x86_64" ]; then
        CMAKE_ARGS+=("-DDENLAB_ARCH=x86_64")
        return 0
    fi
    if [ "$TARGET" = "riscv64" ]; then
        CMAKE_ARGS+=("-DDENLAB_ARCH=riscv64")
        return 0
    fi
...
create_build_dir() {
    if [ "$TARGET" != "lagom" ]; then
        cmake -GNinja "${CMAKE_ARGS[@]}" -S "$DENLAB_SOURCE_DIR/Meta/CMake/Superbuild" -B "$SUPER_BUILD_DIR"
    else
        cmake -GNinja "${CMAKE_ARGS[@]}" -S "$DENLAB_SOURCE_DIR/Meta/Lagom" -B "$SUPER_BUILD_DIR"
    fi
}
...
cmd_with_target() {
    ...
    if [ ! -d "$DENLAB_SOURCE_DIR" ]; then
        DENLAB_SOURCE_DIR="$(get_top_dir)"
        export DENLAB_SOURCE_DIR
    fi
    ...
    BUILD_DIR="$DENLAB_SOURCE_DIR/Build/$TARGET$TARGET_TOOLCHAIN"
    if [ "$TARGET" != "lagom" ]; then
        export DENLAB_ARCH="$TARGET"
        export DENLAB_TOOLCHAIN="$TOOLCHAIN_TYPE"
        if [ "$TOOLCHAIN_TYPE" = "Clang" ]; then
            TOOLCHAIN_DIR="$DENLAB_SOURCE_DIR/Toolchain/Local/clang"
        else
            TOOLCHAIN_DIR="$DENLAB_SOURCE_DIR/Toolchain/Local/$TARGET_TOOLCHAIN/$TARGET"
        fi
        JAKT_TOOLCHAIN_DIR="$DENLAB_SOURCE_DIR/Toolchain/Local/jakt"
        SUPER_BUILD_DIR="$DENLAB_SOURCE_DIR/Build/superbuild-$TARGET$TARGET_TOOLCHAIN"
        JAKT_LIB_DIR="$BUILD_DIR/Root/usr/local/lib/$TARGET-pc-denlab-unknown"
    else
        SUPER_BUILD_DIR="$BUILD_DIR"
        CMAKE_ARGS+=("-DCMAKE_INSTALL_PREFIX=$DENLAB_SOURCE_DIR/Build/lagom-install")
        CMAKE_ARGS+=("-DDENLAB_CACHE_DIR=${DENLAB_SOURCE_DIR}/Build/caches")
    fi
    export PATH="$DENLAB_SOURCE_DIR/Toolchain/Local/cmake/bin":$PATH
}
...
build_target() {
    if [ "$TARGET" = "lagom" ]; then
        ...
        # invoked superbuild for denlab target that doesn't set -DBUILD_LAGOM=ON
        local EXTRA_CMAKE_ARGS=()
        if [ "${CMD_ARGS[0]}" = "ladybird" ]; then
            EXTRA_CMAKE_ARGS=("-DENABLE_LAGOM_LADYBIRD=ON")
        fi
        cmake -S "$DENLAB_SOURCE_DIR/Meta/Lagom" -B "$BUILD_DIR" -DBUILD_LAGOM=ON "${EXTRA_CMAKE_ARGS[@]}"
    fi

    # Get either the environment MAKEJOBS or all processors via CMake
    [ -z "$MAKEJOBS" ] && MAKEJOBS=$(cmake -P "$DENLAB_SOURCE_DIR/Meta/CMake/processor-count.cmake")
...
build_cmake() {
    ...
    ( cd "$DENLAB_SOURCE_DIR/Toolchain" && ./BuildCMake.sh )
}
...
build_toolchain() {
    ...
    if [ "$TOOLCHAIN_TYPE" = "Clang" ]; then
        ( cd "$DENLAB_SOURCE_DIR/Toolchain" && ./BuildClang.sh )
    else
        (
            ...
            cd "$DENLAB_SOURCE_DIR/Toolchain" && ARCH="$TARGET" ./BuildGNU.sh
        )
    fi
}
...
build_jakt() {
    ...
    ( cd "$DENLAB_SOURCE_DIR/Toolchain" && ./BuildJakt.sh )
}
...
ensure_toolchain() {
    if [ "$(cmake -P "$DENLAB_SOURCE_DIR"/Meta/CMake/cmake-version.cmake)" -ne 1 ]; then
        build_cmake
    fi
    ...
    if [ "$TOOLCHAIN_TYPE" = "GNU" ]; then
        local ld_version
        ld_version="$("$TOOLCHAIN_DIR"/bin/"$TARGET"-pc-denlab-ld -v)"
...
run_gdb() {
    ...
    if [ "$TARGET" = "lagom" ]; then
        ...
        if [ -n "$KERNEL_CMD_LINE" ]; then
            export DENLAB_KERNEL_CMDLINE="$KERNEL_CMD_LINE"
        fi
        sleep 1
...
if [[ "$CMD" =~ ^(build|install|image|copy-src|run|gdb|test|rebuild|recreate|kaddr2line|addr2line|setup-and-run)$ ]]; then
    ...
    case "$CMD" in
        ...
        run)
            if [ "$TARGET" = "lagom" ]; then
                ...
                if [ -n "${CMD_ARGS[0]}" ]; then
                    export DENLAB_KERNEL_CMDLINE="${CMD_ARGS[0]}"
                fi
                build_target run
            fi
            ;;
        gdb)
            if [ "$TARGET" = "lagom" ]; then
                ...
                TMUX_SESSION="tmux-denlab-gdb-$(date +%s)"
        ...
        test)
            ...
            if [ "$TARGET" = "lagom" ]; then
                ...
                export DENLAB_KERNEL_CMDLINE="graphics_subsystem_mode=off system_mode=self-test"
                export DENLAB_RUN="ci"
                build_target run
            fi
            ;;
        rebuild)
        ...
        kaddr2line)
            ...
            if [ "$TOOLCHAIN_TYPE" = "Clang" ]; then
                ...
                ADDR2LINE="$TOOLCHAIN_DIR/bin/$TARGET-pc-denlab-addr2line"
            fi
    ...
    if [ "$CMD" = "run" ]; then
        ...
        export DENLAB_EXTRA_QEMU_ARGS="${DENLAB_EXTRA_QEMU_ARGS} -no-reboot -no-shutdown -S"
        # We need to disable kaslr to let gdb map the kernel symbols correctly
        export DENLAB_KERNEL_CMDLINE="${DENLAB_KERNEL_CMDLINE} disable_kaslr"
        set_tmux_title 'qemu'
        build_target run
    elif [ "$CMD" = "gdb" ]; then
        set_tmux_title 'gdb'
