...
if [ "$#" = "0" ]; then
    ...
    echo "This script runs DenLab and exports a set of manpages through ArgsParser,"
...
if [ -z "$BUILD_DIR" ]; then
    if [ -z "$DENLAB_ARCH" ]; then
        export DENLAB_ARCH="x86_64"
        echo "DENLAB_ARCH not given. Assuming ${DENLAB_ARCH}."
    fi
    BUILD_DIR=Build/"$DENLAB_ARCH"
...
export DENLAB_KERNEL_CMDLINE="graphics_subsystem_mode=off panic=shutdown system_mode=generate-manpages"
