...
# Copyright (c) 2018-2023, the DenLabOS developers.
...
BUILD_DIRECTORY = Path(environ.get("DENLAB_BUILD_DIR") or Path.cwd())
...
@unique
class Arch(Enum):
    """DenLabOS architecture, not host architecture."""
...
class QEMUKind(Enum):
    ...
    # MacOS with HVF.
    MacOS = "hvf"
    # DenLab on DenLab with ported QEMU
    DenLabOS = "tcg"
...
def determine_qemu_kind() -> QEMUKind:
    if which("wslpath") is not None and environ.get("DENLAB_NATIVE_WINDOWS_QEMU", "1") == "1":
    ...
    if os.uname().sysname == "DenLabOS":
        return QEMUKind.DenLabOS
    return QEMUKind.Other


def determine_denlab_arch() -> Arch:
    arch = environ.get("DENLAB_ARCH")
    if arch == "aarch64":
        return Arch.Aarch64
    if arch == "riscv64":
        return Arch.RISCV64
    if arch == "x86_64":
        return Arch.x86_64
    raise RunError("Please specify a valid DenLabOS architecture")


def determine_machine_type() -> MachineType:
    provided_machine_type = environ.get("DENLAB_RUN")
    if provided_machine_type is not None:
        try:
            value = MachineType(provided_machine_type)
        except ValueError:
            raise RunError(f"{provided_machine_type} is not a valid DenLabOS machine type")
        return value
    return MachineType.Default


def determine_boot_drive_type() -> BootDriveType:
    provided_boot_drive_type = environ.get("DENLAB_BOOT_DRIVE")
    if provided_boot_drive_type is not None:
        try:
            value = BootDriveType(provided_boot_drive_type)
        except ValueError:
            raise RunError(f"{provided_boot_drive_type} is not a valid DenLabOS boot drive type")
        return value
    return BootDriveType.NVMe


def detect_ram_size() -> str | None:
    return environ.get("DENLAB_RAM_SIZE", "2G")


def set_up_qemu_binary(config: Configuration):
    qemu_binary_basename: str | None = None
    if "DENLAB_QEMU_BIN" in environ:
        qemu_binary_basename = environ.get("DENLAB_QEMU_BIN")
    else:
    ...
    if config.qemu_binary is None:
        ...
        local_qemu_bin = Path(str(config.denlab_src), "Toolchain/Local/qemu/bin/", qemu_binary_basename)
        old_local_qemu_bin = Path(str(config.denlab_src), "Toolchain/Local/x86_64/bin/", qemu_binary_basename)
...
def set_up_virtualization_support(config: Configuration):
    provided_virtualization_enable = environ.get("DENLAB_VIRTUALIZATION_SUPPORT")
...
def set_up_basic_kernel_cmdline(config: Configuration):
    provided_cmdline = environ.get("DENLAB_KERNEL_CMDLINE")
...
def set_up_disk_image_path(config: Configuration):
    provided_disk_image = environ.get("DENLAB_DISK_IMAGE")
...
def set_up_cpu(config: Configuration):
    if config.qemu_kind == QEMUKind.NativeWindows:
        ...
        provided_cpu = environ.get("DENLAB_QEMU_CPU")
...
def set_up_cpu_count(config: Configuration):
    ...
    provided_cpu_count = environ.get("DENLAB_CPUS")
...
def set_up_spice(config: Configuration):
    ...
    if config.machine_type != MachineType.Default:
        return
    use_non_qemu_spice = environ.get("DENLAB_SPICE") == "1"
    chardev_info = run(
...
def set_up_audio_hardware(config: Configuration):
    provided_audio_hardware = environ.get("DENLAB_AUDIO_HARDWARE", "intelhda")
...
def set_up_screens(config: Configuration):
    provided_screen_count_unparsed = environ.get("DENLAB_SCREENS", "1")
    try:
    ...
    provided_display_backend = environ.get("DENLAB_QEMU_DISPLAY_BACKEND")
    if provided_display_backend is not None:
        ...
        if len(config.spice_arguments) != 0:
            ...
            # https://github.com/DenLabOS/denlab/issues/7657
        ...
        elif config.qemu_kind == QEMUKind.DenLabOS:
            config.display_backend = "sdl,gl=off"
        else:
...
def set_up_display_device(config: Configuration):
    config.enable_gl = environ.get("DENLAB_GL") == "1"
    provided_display_device = environ.get("DENLAB_QEMU_DISPLAY_DEVICE")
    if provided_display_device is not None:
        ...
        if config.qemu_kind == QEMUKind.MacOS:
            raise RunError("DENLAB_GL is not supported since there's no GL backend on macOS")
        elif config.screen_count > 1:
            raise RunError("DENLAB_GL and multi-monitor support cannot be set up simultaneously")
...
def determine_host_address() -> str:
    return environ.get("DENLAB_HOST_IP", "127.0.0.1")


def set_up_gdb(config: Configuration):
    config.enable_gdb = environ.get("DENLAB_DISABLE_GDB_SOCKET") != "1"
...
def set_up_network_hardware(config: Configuration):
    config.packet_logging_arguments = (environ.get("DENLAB_PACKET_LOGGING_ARG", "")).split()

    provided_ethernet_device_type = environ.get("DENLAB_ETHERNET_DEVICE_TYPE")
...
def set_up_machine_devices(config: Configuration):
    ...
    config.extra_arguments.extend(["-name", "DenLabOS", "-d", "guest_errors"])
...
def assemble_arguments(config: Configuration) -> list[str | Path]:
    passed_qemu_args = shlex.split(environ.get("DENLAB_EXTRA_QEMU_ARGS", ""))
...
def configure_and_run():
    ...
    config.architecture = determine_denlab_arch()
    ...
    denlab_src = environ.get("DENLAB_SOURCE_DIR")
    if denlab_src is None:
        raise RunError("DENLAB_SOURCE_DIR not set or empty")
    config.denlab_src = Path(denlab_src)
