...
if [ -z "${DENLAB_STRIPPED_ENV:-}" ]; then
    exec "${SCRIPT}/.strip_env.sh" "${@}"
fi
unset DENLAB_STRIPPED_ENV
...
target_env() {
    ...
    elif [ "$(uname -s)" = "DenLabOS" ]; then
        export DENLAB_ARCH="$(uname -m)"
        export DENLAB_INSTALL_ROOT=""
    else
        >&2 echo "Error: .hosted_defs.sh is missing and we are not running on DenLab."
        exit 1
    fi
}
...
PORT_META_DIR="$(pwd)"
if [[ -z ${DENLAB_BUILD_DIR:-} ]]; then
    PORT_BUILD_DIR="${PORT_META_DIR}"
else
    PORT_BUILD_DIR="${DENLAB_BUILD_DIR}/Ports/${port}"
fi
...
get_new_config_sub() {
    ...
    if ! run grep -q denlab "$config_sub"; then
...
get_new_config_guess() {
    ...
    if ! run grep -q DenLabOS "$config_guess"; then
...
ensure_build() {
    ...
    if [ ! -f "${DESTDIR}/usr/lib/libc.so" ]; then
        echo "libc.so could not be found. This likely means that DenLabOS:"
        echo "- has not been built and/or installed yet"
        echo "- has been installed in an unexpected location"
        echo "The currently configured build directory is ${DENLAB_BUILD_DIR}. Resolve this issue and try again."
        exit 1
    fi
}
...
install_icon() {
    ...
    run $OBJCOPY --add-section denlab_icon_s="app-16x16.png" "${DESTDIR}${launcher}"
    run $OBJCOPY --add-section denlab_icon_m="app-32x32.png" "${DESTDIR}${launcher}"
}
...
func_defined configure || configure() {
    ...
    if [[ -n "${DENLAB_SOURCE_DIR:-}" ]]; then
        run ./"$configscript" --host="${DENLAB_ARCH}-pc-denlab" "${configopts[@]}"
    else
        run ./"$configscript" --build="${DENLAB_ARCH}-pc-denlab" "${configopts[@]}"
    fi
}
...
installdepends() {
    for depend in "${depends[@]}"; do
        ...
        IFS=':' read -ra port_directories <<< "$DENLAB_PORT_DIRS"
...
do_generate_patch_readme() {
    ...
    echo "# Patches for $port on DenLabOS" > ReadMe.md
    echo >> ReadMe.md
...
do_dev() {
    if [ -n "${IN_DENLAB_PORT_DEV:-}"  ]; then
        >&2 echo "Error: Already in dev environment for $IN_DENLAB_PORT_DEV"
        exit 1
    fi
