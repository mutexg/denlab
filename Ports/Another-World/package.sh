...
configopts=("-DCMAKE_TOOLCHAIN_FILE=${DENLAB_BUILD_DIR}/CMakeToolchain.txt" "-DSDL2_INCLUDE_DIR=${DENLAB_INSTALL_ROOT}/usr/local/include/SDL2")
files=(
...
install() {
    run mkdir -p "${DENLAB_INSTALL_ROOT}/opt/Another-World"
    run cp -r raw "${DENLAB_INSTALL_ROOT}/opt/Another-World"
