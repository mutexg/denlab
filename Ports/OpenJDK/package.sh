...
configure() {
    ...
    run bash configure \
        ...
        --openjdk-target=${DENLAB_ARCH}-pc-denlab \
        --with-sysroot=${DENLAB_INSTALL_ROOT} \
        --with-jvm-variants=zero \
        --enable-headless-only \
        --with-debug-level=fastdebug \
        --with-native-debug-symbols=internal \
        --with-tools-dir=${DENLAB_TOOLCHAIN_BINDIR} \
...
install() {
    run mkdir -p ${DENLAB_INSTALL_ROOT}/usr/local/lib/jvm/
    run sh -c "cp ./build/denlab-* ${DENLAB_INSTALL_ROOT}/usr/local/lib/jvm/ -rf"
}
