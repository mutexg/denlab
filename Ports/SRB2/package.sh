...
install() {
    run /usr/bin/install -d "${DENLAB_INSTALL_ROOT}${install_dir}/"
    run /usr/bin/install ./build/bin/srb2 ./srb2.png "${DENLAB_INSTALL_ROOT}${install_dir}"
    run cp -r ${PWD}/*.{dat,dta,pk3,txt} "${DENLAB_INSTALL_ROOT}${install_dir}/"
    run cp -r "../models" "${DENLAB_INSTALL_ROOT}${install_dir}/"
}
