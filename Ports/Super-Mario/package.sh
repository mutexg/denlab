...
install() {
    run mkdir -p "${DENLAB_INSTALL_ROOT}/opt/Super_Mario"
    run cp -r uMario files "${DENLAB_INSTALL_ROOT}/opt/Super_Mario"
    if command -v convert >/dev/null; then
        run convert "app.ico[0]" app-16x16.png
        run convert "app.ico[1]" app-32x32.png
        run $OBJCOPY --add-section denlab_icon_s="app-16x16.png" "${DENLAB_INSTALL_ROOT}/opt/Super_Mario/uMario"
        run $OBJCOPY --add-section denlab_icon_m="app-32x32.png" "${DENLAB_INSTALL_ROOT}/opt/Super_Mario/uMario"
    fi
}
