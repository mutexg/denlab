...
configure() {
    run ./"$configscript" \
        ...
        --with-ssl-lib-dir="${DENLAB_INSTALL_ROOT}/usr/local/lib" \
        --with-include-path="${DENLAB_INSTALL_ROOT}/usr/local/include:${DENLAB_INSTALL_ROOT}/usr/local/include/openssl" \
...
install() {
    ...
    run /usr/bin/install \
        ...
        --no-target-directory \
        -D \
        pine-system.denlab.conf "${DENLAB_INSTALL_ROOT}/etc/pine.conf"
}
