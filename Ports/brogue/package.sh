...
install() {
    datadir="${DENLAB_INSTALL_ROOT}/usr/local/share/games/brogue/assets"
    mkdir -p "${DENLAB_INSTALL_ROOT}/usr/local/bin"
    mkdir -p "${datadir}"
    cp "${workdir}"/bin/assets/* "${datadir}"
    cp "${workdir}/bin/brogue" "${DENLAB_INSTALL_ROOT}/usr/local/bin"
}
