...
configure() {
    ...
    run sh -c "cd target-build && ../configure --host="${DENLAB_ARCH}-pc-denlab" --disable-helpers ${configopts[@]} CFLAGS=-I."
}
...
install() {
    run sh -c "cd target-build && make DESTDIR="${DENLAB_INSTALL_ROOT}" ${installopts[@]} install"
}
