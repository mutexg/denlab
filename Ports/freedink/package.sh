...
install() {
    run_nocd make DESTDIR="${DENLAB_INSTALL_ROOT}" -C "${freedink_data}" install
    run make DESTDIR="${DENLAB_INSTALL_ROOT}" "${installopts[@]}" install
}

export CPPFLAGS="-I${DENLAB_INSTALL_ROOT}/usr/local/include/SDL2 -I${DENLAB_INSTALL_ROOT}/usr/local/include/libxml2"
