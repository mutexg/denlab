...
export PKG_CONFIG_PATH="${DENLAB_INSTALL_ROOT}/usr/local/lib/pkgconfig"
...
install() {
    run ./waf install --destdir=${DENLAB_INSTALL_ROOT}/home/anon/Games/halflife
}
...
post_install() {
    ...
    pushd ${DENLAB_INSTALL_ROOT}/home/anon/Games/halflife/valve/cl_dlls/
    "$rename_command" 's/^...//' lib*
    popd
    pushd ${DENLAB_INSTALL_ROOT}/home/anon/Games/halflife/valve/dlls/
    "$rename_command" 's/^...//' lib*
    popd

    # Create a launch script
    cat <<- 'EOF' > ${DENLAB_INSTALL_ROOT}/home/anon/Games/halflife/hl.sh
#!/bin/sh
...
./xash3d -console
EOF
    chmod +x ${DENLAB_INSTALL_ROOT}/home/anon/Games/halflife/hl.sh

    echo "Please remember to copy the 'valve/' folder from your own Half-Life installation"
    echo "into ${DENLAB_INSTALL_ROOT}/home/anon/Games/halflife/"
}
