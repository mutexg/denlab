...
makeopts=(
    ...
    'WITHOUT_GTK=1'
    'USE_POLYMOST=1'
    "SDL2CONFIG=${DENLAB_BUILD_DIR}/Root/usr/local/bin/sdl2-config"
)
...
install() {
    run mkdir -p "${DENLAB_INSTALL_ROOT}${install_dir}"
    run cp build "${DENLAB_INSTALL_ROOT}${install_dir}"
    run cp duke3d "${DENLAB_INSTALL_ROOT}${install_dir}"
}
...
post_install() {
  ...
  echo '    Inside DenLabOS: ~/.jfduke3d'
  echo "    Outside DenLabOS: ${DENLAB_INSTALL_ROOT}/Base/home/anon/.jfduke3d"
  echo
