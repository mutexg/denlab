...
post_install() {
    ...
    for lib in tinfo tic curses; do
        ln -svf libncursesw.so "${DENLAB_INSTALL_ROOT}/usr/local/lib/lib${lib}w.so"
    done

    # Compatibility symlinks for non-w libraries.
    for lib in form menu ncurses ncurses++ panel tinfo tic curses; do
        ln -svf "lib${lib}w.so" "${DENLAB_INSTALL_ROOT}/usr/local/lib/lib${lib}.so"
    done
    ...
    rm -rf "${DENLAB_INSTALL_ROOT}/usr/local/include/ncurses"
    ln -svf ncursesw "${DENLAB_INSTALL_ROOT}/usr/local/include/ncurses"
    ...
    for file in $(ls "${DENLAB_INSTALL_ROOT}/usr/local/include/ncursesw"); do
        ln -svf "ncursesw/${file}" "${DENLAB_INSTALL_ROOT}/usr/local/include/${file}"
    done
}
