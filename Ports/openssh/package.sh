...
configopts=("--prefix=/usr/local" "--disable-utmp" "--disable-strip" "--sysconfdir=/etc/ssh" "--with-ssl-dir=${DENLAB_INSTALL_ROOT}/usr/local/lib")
...
install() {
    # Can't make keys outside of DenLab since ssh-keygen is built for DenLab.
    run make DESTDIR="${DENLAB_INSTALL_ROOT}" "${installopts[@]}" install-nokeys
}
