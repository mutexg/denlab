...
configopts=(
    ...
    '--disable-opcache'
    '--enable-fpm'
    "--prefix=${DENLAB_INSTALL_ROOT}/usr/local"
    '--with-curl'
    "--with-iconv=${DENLAB_INSTALL_ROOT}/usr/local"
    '--with-openssl'
    "--with-readline=${DENLAB_INSTALL_ROOT}/usr/local"
    '--with-zlib'
    '--without-pcre-jit'
)
launcher_name='PHP'
...
export CFLAGS="-I${DENLAB_INSTALL_ROOT}/usr/include/LibCrypt -I${DENLAB_INSTALL_ROOT}/usr/local/include/libxml2"
export LIBS='-ldl'
