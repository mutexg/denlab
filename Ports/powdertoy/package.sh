...
version=96.2.350
useconfigure=true
configopts=("-Dbuildtype=release" "build-release" "--cross-file" "${DENLAB_BUILD_DIR}/meson-cross-file.txt")
...
install() {
    run cp build-release/powder "${DENLAB_INSTALL_ROOT}/usr/local/bin"
}
