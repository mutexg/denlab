...
export PATH="${DENLAB_SOURCE_DIR}/Toolchain/Local/python/bin:${PATH}"
...
configure() {
    run ./configure --host="${DENLAB_ARCH}-pc-denlab" "--with-build-python=${PYTHON_BIN}" --build="$($workdir/config.guess)" "${configopts[@]}"
}
