...
build() {
    export CC="${DENLAB_SOURCE_DIR}/Toolchain/Local/${DENLAB_ARCH}/bin/${DENLAB_ARCH}-pc-denlab-gcc"
    run make CC="${CC}"
}

install() {
    run cp qopconv "${DENLAB_INSTALL_ROOT}/bin"

    target_dir="${DENLAB_INSTALL_ROOT}/usr/local/include/"
