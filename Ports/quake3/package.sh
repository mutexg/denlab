...
install() {
    run make COPYDIR=${DENLAB_INSTALL_ROOT}${install_dir}/ copyfiles
}
...
post_install() {
    ...
    echo "${bin_path}	${bin_path}	bind	bind,nodev,nosuid,wxallowed" > "${DENLAB_INSTALL_ROOT}/etc/fstab.d/${port}"

    # Let's create a more DenLab friendly `autoexec.cfg` file :^)
    cat <<- 'EOF' > ${DENLAB_INSTALL_ROOT}${install_dir}/baseq3/autoexec.cfg
