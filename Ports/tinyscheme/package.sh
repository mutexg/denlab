...
install() {
    run mkdir -p "${DENLAB_BUILD_DIR}/Root/usr/local/bin"
    run cp scheme "${DENLAB_BUILD_DIR}/Root/usr/local/bin/tinyscheme"
    run mkdir -p "${DENLAB_BUILD_DIR}/Root/usr/local/include/tinyscheme"
    run cp init.scm "${DENLAB_BUILD_DIR}/Root/usr/local/include/tinyscheme/init.scm"
}
