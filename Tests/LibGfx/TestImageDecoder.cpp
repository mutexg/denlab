 ...
 * Copyright (c) 2021, Brian Gianforcaro <bgianf@denlabos.org>
 *
...
{
    auto file = TRY_OR_FAIL(Core::MappedFile::map(TEST_INPUT("ico/denlab.ico"sv)));
    EXPECT(Gfx::ICOImageDecoderPlugin::sniff(file->bytes()));
    auto plugin_decoder = TRY_OR_FAIL(Gfx::ICOImageDecoderPlugin::create(file->bytes()));
...
{
    auto file = TRY_OR_FAIL(Core::MappedFile::map(TEST_INPUT("ilbm/denlab.lbm"sv)));
    EXPECT(Gfx::ILBMImageDecoderPlugin::sniff(file->bytes()));
    auto plugin_decoder = TRY_OR_FAIL(Gfx::ILBMImageDecoderPlugin::create(file->bytes()));
...
{
    auto file = TRY_OR_FAIL(Core::MappedFile::map(TEST_INPUT("ilbm/denlab-24bit.iff"sv)));
    EXPECT(Gfx::ILBMImageDecoderPlugin::sniff(file->bytes()));
    auto plugin_decoder = TRY_OR_FAIL(Gfx::ILBMImageDecoderPlugin::create(file->bytes()));
