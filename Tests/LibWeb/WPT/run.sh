...
then
    DENLAB_SOURCE_DIR="$(realpath "${SCRIPT_DIR}/../../../")"
    export DENLAB_SOURCE_DIR
fi


: "${WEBDRIVER_BINARY:=$(env PATH="${DENLAB_SOURCE_DIR}/Build/lagom/bin/Ladybird.app/Contents/MacOS:${DENLAB_SOURCE_DIR}/Build/lagom/bin:${DENLAB_SOURCE_DIR}/Meta/Lagom/Build/bin:${PATH}" \
                         which WebDriver)}"
...
python3 ./wpt/wpt run ladybird \
                  ...
                  --webdriver-arg="--certificate=${DENLAB_SOURCE_DIR}/Build/lagom/cacert.pem" \
