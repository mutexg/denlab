...
for arch in $ARCHS; do
    pushd "$DIR/.."
        cmake -DDENLAB_ARCH="$arch" -DDENLAB_SYSROOT="$BUILD/${arch}clang/Root" -P Meta/CMake/link_libc_headers.cmake
    popd
done
...
pushd "$DIR/Build/clang"
    ...
    pushd llvm
        buildstep "llvm/configure" cmake "$DIR/Tarballs/$LLVM_NAME/llvm" \
            -G Ninja \
            -DDENLAB_x86_64-pc-denlab_SYSROOT="$BUILD/x86_64clang/Root" \
            -DDENLAB_aarch64-pc-denlab_SYSROOT="$BUILD/aarch64clang/Root" \
            -DDENLAB_riscv64-pc-denlab_SYSROOT="$BUILD/riscv64clang/Root" \
            -DDENLAB_x86_64-pc-denlab_STUBS="$DIR/Stubs/x86_64" \
            -DDENLAB_aarch64-pc-denlab_STUBS="$DIR/Stubs/aarch64" \
            -DDENLAB_riscv64-pc-denlab_STUBS="$DIR/Stubs/riscv64" \
...
pushd "$DIR/Local/clang/bin/"
    ...
    for arch in $ARCHS; do
        ln -s clang "$arch"-pc-denlab-clang
        ln -s clang++ "$arch"-pc-denlab-clang++
        ln -s llvm-nm "$arch"-pc-denlab-nm
        echo "--sysroot=$BUILD/${arch}clang/Root" > "$arch"-pc-denlab.cfg
    done
popd
