...
ErrorOr<int> denlab_main(Main::Arguments arguments)
{
    ...
    auto show_users_action = GUI::Action::create_checkable(
        "Show DenLabOS &users", TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/ladyball.png"sv)), [&map_widget](auto& action) { map_widget.set_show_users(action.is_checked()); }, window);
