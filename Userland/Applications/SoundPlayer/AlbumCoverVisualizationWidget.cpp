...
void AlbumCoverVisualizationWidget::paint_event(GUI::PaintEvent& event)
{
    ...
    if (cover) {
        ...
        if (!m_denlab_bg)
            m_denlab_bg = Gfx::Bitmap::load_from_file("/res/wallpapers/sunset-retro.png"sv).release_value_but_fixme_should_propagate_errors();
        painter.draw_scaled_bitmap(frame_inner_rect(), *m_denlab_bg, m_denlab_bg->rect(), 1.0f);
    }
}
