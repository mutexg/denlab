 ...
 * Copyright (c) 2024, Sam Atkins <atkinssj@denlabos.org>
 *
...
ByteString HackStudioWidget::get_full_path_of_denlab_source(ByteString const& file)
{
    ...
    constexpr char DENLAB_LIBS_PREFIX[] = "/usr/src/denlab";
    LexicalPath denlab_sources_base(DENLAB_LIBS_PREFIX);
    return ByteString::formatted("{}/{}", denlab_sources_base, relative_path_builder.to_byte_string());
}
...
ByteString HackStudioWidget::get_absolute_path(ByteString const& path) const
{
    ...
    if (path.starts_with(".."sv)) {
        return get_full_path_of_denlab_source(path);
    }
...
RefPtr<EditorWrapper> HackStudioWidget::get_editor_of_file(ByteString const& filename)
{
    ...
    if (filename.starts_with("../"sv)) {
        file_path = get_full_path_of_denlab_source(filename);
    }
