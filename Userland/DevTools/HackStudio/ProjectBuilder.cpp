...
ProjectBuilder::ProjectBuilder(NonnullRefPtr<TerminalWrapper> terminal, Project const& project)
    ...
    , m_is_denlab(project.project_is_denlab() ? IsDenLabRepo::Yes : IsDenLabRepo::No)
{
}
...
ErrorOr<void> ProjectBuilder::build(StringView active_file)
{
    ...
    if (m_is_denlab == IsDenLabRepo::No) {
...
ErrorOr<void> ProjectBuilder::run(StringView active_file)
{
    ...
    return run_denlab_component();
}

ErrorOr<void> ProjectBuilder::run_denlab_component()
{
    auto relative_path_to_dir = LexicalPath::relative_path(LexicalPath::dirname(m_denlab_component_cmake_file), m_project_root);
    TRY(m_terminal->run_command(LexicalPath::join(relative_path_to_dir, m_denlab_component_name).string(), build_directory()));
    return {};
}
...
ErrorOr<void> ProjectBuilder::update_active_file(StringView active_file)
{
    ...
    if (m_denlab_component_cmake_file == cmake_file.value())
        return {};

    m_denlab_component_cmake_file = cmake_file.value();
    m_denlab_component_name = TRY(component_name(m_denlab_component_cmake_file));

    TRY(initialize_build_directory());
    return {};
}

ErrorOr<void> ProjectBuilder::build_denlab_component()
{
    TRY(verify_make_is_installed());
    TRY(m_terminal->run_command(ByteString::formatted("make {}", m_denlab_component_name), build_directory(), TerminalWrapper::WaitForExit::Yes, "Make failed"sv));
    return {};
}
...
ErrorOr<ByteString> ProjectBuilder::component_name(StringView cmake_file_path)
{
    ...
    static Regex<ECMA262> const component_name(R"~~~(denlab_component\([\s]*(\w+)[\s\S]*\))~~~");
    RegexResult result;
...
ByteString ProjectBuilder::generate_cmake_file_content() const
{
    ...
    builder.appendff("add_subdirectory({})\n", LexicalPath::dirname(m_denlab_component_cmake_file));
...
{
    Vector<ByteString> arguments = { "-c", "find Userland -name CMakeLists.txt | xargs grep denlab_lib" };
    ...
    static Regex<ECMA262> const parse_library_definition(R"~~~(.+:denlab_lib[c]?\((\w+) (\w+)\).*)~~~");
