...
ErrorOr<void> ChessWidget::export_pgn(Core::File& file) const
{
    ...
    TRY(file.write_until_depleted("[Site \"DenLabOS Chess\"]\n"sv.bytes()));
    ...
    auto const player2 = (!m_engine.is_null() ? "DenLabOS ChessEngine"sv : "?"sv);
    ...
    TRY(file.write_until_depleted("[Annotator \"DenLabOS Chess\"]\n"sv.bytes()));
