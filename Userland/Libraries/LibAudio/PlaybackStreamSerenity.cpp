...
#include "PlaybackStreamDenLab.h"

#include <LibCore/ThreadedPromise.h>

namespace Audio {

ErrorOr<NonnullRefPtr<PlaybackStream>> PlaybackStreamDenLab::create(OutputState initial_state, u32 sample_rate, u8 channels, [[maybe_unused]] u32 target_latency_ms, AudioDataRequestCallback&& data_request_callback)
{
    ...
    auto implementation = TRY(adopt_nonnull_ref_or_enomem(new (nothrow) PlaybackStreamDenLab(connection, move(polling_timer), move(data_request_callback))));
    if (initial_state == OutputState::Playing)
        connection->async_start_playback();
    return implementation;
}

PlaybackStreamDenLab::PlaybackStreamDenLab(NonnullRefPtr<ConnectionToServer> stream, NonnullRefPtr<Core::Timer> polling_timer, AudioDataRequestCallback&& data_request_callback)
...
void PlaybackStreamDenLab::fill_buffers()
{
...
void PlaybackStreamDenLab::set_underrun_callback(Function<void()> callback)
{
    // FIXME: Implement underrun callback in AudioServer
    (void)callback;
}

NonnullRefPtr<Core::ThreadedPromise<Duration>> PlaybackStreamDenLab::resume()
{
...
NonnullRefPtr<Core::ThreadedPromise<void>> PlaybackStreamDenLab::drain_buffer_and_suspend()
{
...
NonnullRefPtr<Core::ThreadedPromise<void>> PlaybackStreamDenLab::discard_buffer_and_suspend()
{
    auto promise = Core::ThreadedPromise<void>::create();
...
ErrorOr<Duration> PlaybackStreamDenLab::total_time_played()
{
    return Duration::from_milliseconds(m_number_of_samples_enqueued * 1000 / m_connection->get_self_sample_rate());
}

NonnullRefPtr<Core::ThreadedPromise<void>> PlaybackStreamDenLab::set_volume(double volume)
{
    auto promise = Core::ThreadedPromise<void>::create();
