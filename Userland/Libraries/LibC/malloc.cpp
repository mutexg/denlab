 ...
 * Copyright (c) 2022, Peter Elliott <pelliott@denlabos.org>
 *
...
static ErrorOr<void*> os_alloc(size_t size, char const* name)
{
    ...
    auto* ptr = denlab_mmap(nullptr, size, PROT_READ | PROT_WRITE, flags, 0, 0, ChunkedBlock::block_size, name);
    VERIFY(ptr != nullptr);
    if (ptr == MAP_FAILED) {
        return ENOMEM;
    }
