...
ErrorOr<void> DebugInfo::prepare_lines()
{
    ...
    auto compute_full_path = [&](DeprecatedFlyString const& file_path) -> Optional<ByteString> {
        ...
        if (auto index_of_denlab_slash = file_path.view().find("denlab/"sv); index_of_denlab_slash.has_value()) {
            auto start_index = index_of_denlab_slash.value() + "denlab/"sv.length();
