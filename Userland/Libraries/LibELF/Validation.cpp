/*
 * Copyright (c) 2020, Andrew Kaster <akaster@denlabos.org>
...
#include <Kernel/API/denlab_limits.h>
...
bool validate_program_headers(Elf_Ehdr const& elf_header, size_t file_size, ReadonlyBytes buffer, Optional<Elf_Phdr>& interpreter_path_program_header, Optional<size_t>* requested_stack_size, bool verbose)
{
    ...
    for (size_t header_index = 0; header_index < num_program_headers; ++header_index) {
        ...
        if (elf_header.e_type != ET_CORE) {
            ...
            if (program_header.p_type == PT_LOAD && program_header.p_align % (size_t)DENLAB_PAGE_SIZE != 0) {
                if (verbose)
                    dbgln("Program header ({}) with p_type PT_LOAD has p_align ({}) not divisible by page size ({})", header_index, program_header.p_align, DENLAB_PAGE_SIZE);
                return false;
            }
        ...
        case PT_GNU_STACK:
            ...
            if (program_header.p_memsz != 0) {
                ...
                if (program_header.p_memsz % DENLAB_PAGE_SIZE != 0) {
                    if (verbose)
