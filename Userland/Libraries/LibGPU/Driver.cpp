/*
 * Copyright (c) 2022, Stephan Unverwerth <s.unverwerth@denlabos.org>
...
#if defined(AK_OS_DENLAB)
    { "softgpu"sv, "libsoftgpu.so.denlab" },
    { "virtgpu"sv, "libvirtgpu.so.denlab" },
...
ErrorOr<NonnullRefPtr<Driver>> Driver::try_create(StringView driver_name)
{
    ...
    auto denlab_gpu_create_device = reinterpret_cast<denlab_gpu_create_device_t>(dlsym(lib, "denlab_gpu_create_device"));
    if (!denlab_gpu_create_device) {
        dlclose(lib);
        return Error::from_string_literal("The library for the requested GPU driver does not contain denlab_gpu_create_device()");
    }

    auto driver = adopt_ref(*new Driver(lib, denlab_gpu_create_device));
...
{
    auto device_or_null = m_denlab_gpu_create_device(size);
    if (!device_or_null)
