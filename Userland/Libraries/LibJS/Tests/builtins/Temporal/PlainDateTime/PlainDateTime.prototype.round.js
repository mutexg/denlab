...
describe("errors", () => {
    ...
    test("invalid rounding mode", () => {
        expect(() => {
            const plainDateTime = new Temporal.PlainDateTime(2021, 11, 3, 18, 8, 10, 100, 200, 300);
            plainDateTime.round({ smallestUnit: "second", roundingMode: "denlabOS" });
        }).toThrowWithMessage(
            RangeError,
            "denlabOS is not a valid value for option roundingMode"
        );
    });
    ...
    test("invalid smallest unit", () => {
        expect(() => {
            const plainDateTime = new Temporal.PlainDateTime(2021, 11, 3, 18, 8, 10, 100, 200, 300);
            plainDateTime.round({ smallestUnit: "denlabOS" });
        }).toThrowWithMessage(
            RangeError,
            "denlabOS is not a valid value for option smallestUnit"
        );
    });
