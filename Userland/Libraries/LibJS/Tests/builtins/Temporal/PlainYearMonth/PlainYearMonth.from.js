...
describe("errors", () => {
    ...
    test("can only use iso8601 calendar with year month strings", () => {
        ...
        expect(() => {
            Temporal.PlainYearMonth.from("2023-02[u-ca=DenLabOS]");
        }).toThrowWithMessage(
            RangeError,
            "YYYY-MM string format can only be used with the iso8601 calendar"
        );
    ...
    test("doesn't throw non-iso8601 calendar error when using a superset format string such as DateTime", () => {
        // NOTE: This will still throw, but only because "denlab" is not a recognised calendar, not because of the string format restriction.
        try {
            Temporal.PlainYearMonth.from("2023-02-10T22:57[u-ca=denlab]");
        } catch (e) {
            expect(e).toBeInstanceOf(RangeError);
            expect(e.message).not.toBe(
                "MM-DD string format can only be used with the iso8601 calendar"
            );
            expect(e.message).toBe("Invalid calendar identifier 'denlab'");
        }
    });
});
