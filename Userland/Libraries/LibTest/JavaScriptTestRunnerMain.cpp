/*
 * Copyright (c) 2020, Matthew Olsson <mattco@denlabos.org>
 * Copyright (c) 2020-2021, Linus Groh <linusg@denlabos.org>
 * Copyright (c) 2021, Ali Mohammad Pur <mpfard@denlabos.org>
 *
...
#else
        char* denlab_source_dir = getenv("DENLAB_SOURCE_DIR");
        if (!denlab_source_dir) {
            warnln("No test root given, {} requires the DENLAB_SOURCE_DIR environment variable to be set", g_program_name);
            return 1;
        }
        test_root = ByteString::formatted("{}/{}", denlab_source_dir, g_test_root_fragment);
        common_path = ByteString::formatted("{}/Userland/Libraries/LibJS/Tests/test-common.js", denlab_source_dir);
#endif
    }
