/*
 * Copyright (c) 2022-2023, Tim Flynn <trflynn89@denlabos.org>
 *
...
static bool could_be_start_of_denlab_emoji(u32 code_point)
{
    // We use Supplementary Private Use Area-B for custom DenLab emoji, starting at U+10CD00.
    static constexpr auto first_custom_denlab_emoji_code_point = 0x10CD00u;

    return code_point >= first_custom_denlab_emoji_code_point;
}
...
static bool could_be_start_of_emoji_sequence_impl(CodePointIterator const& it, SequenceType type)
{
    ...
    if (could_be_start_of_emoji_core_sequence(code_point, next_code_point, type))
        return true;
    if (could_be_start_of_denlab_emoji(code_point))
        return true;
    return false;
}
