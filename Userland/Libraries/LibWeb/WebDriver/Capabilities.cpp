...
{
    if (!value.is_object())
        return Error::from_code(ErrorCode::InvalidArgument, "Extension capability denlab:ladybird must be an object"sv);

    auto const& object = value.as_object();

    if (auto headless = object.get("headless"sv); headless.has_value() && !headless->is_bool())
        return Error::from_code(ErrorCode::InvalidArgument, "Extension capability denlab:ladybird/headless must be a boolean"sv);
...
static ErrorOr<JsonObject, Error> validate_capabilities(JsonValue const& capability)
{
    ...
    TRY(capability.as_object().try_for_each_member([&](auto const& name, JsonValue const& value) -> ErrorOr<void, Error> {
        ...
        else if (name == "denlab:ladybird"sv) {
...
static bool matches_platform_name(StringView requested_platform_name, StringView required_platform_name)
{
    ...
    //       Further, we are allowed to handle synonyms for DenLabOS.
    if (requested_platform_name == "mac"sv && required_platform_name == "macos"sv)
        return true;
    if (requested_platform_name == "denlab"sv && required_platform_name == "denlabos"sv)
        return true;
    return false;
}
...
static JsonValue match_capabilities(JsonObject const& capabilities)
{
    ...
    matched_capabilities.set("denlab:ladybird"sv, default_ladybird_options());
...
{
    auto options = capabilities.get_object("denlab:ladybird"sv);
