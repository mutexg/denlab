...
#include <LibWebSocket/Impl/WebSocketImplDenLab.h>

namespace WebSocket {

WebSocketImplDenLab::~WebSocketImplDenLab() = default;
WebSocketImplDenLab::WebSocketImplDenLab() = default;

bool WebSocketImplDenLab::can_read_line()
{
    return MUST(m_socket->can_read_line());
}

bool WebSocketImplDenLab::send(ReadonlyBytes bytes)
{
    return !m_socket->write_until_depleted(bytes).is_error();
}

bool WebSocketImplDenLab::eof()
{
    return m_socket->is_eof();
}

void WebSocketImplDenLab::discard_connection()
{
    m_socket = nullptr;
}

void WebSocketImplDenLab::connect(ConnectionInfo const& connection_info)
{
    VERIFY(!m_socket);
    VERIFY(on_connected);
...
ErrorOr<ByteBuffer> WebSocketImplDenLab::read(int max_size)
{
...
ErrorOr<ByteString> WebSocketImplDenLab::read_line(size_t size)
{
