...
ErrorOr<void> SpiceAgent::send_clipboard_contents(ClipboardDataType data_type)
{
    ...
    // We have an exception for `image/x-denlabos`, where we treat it as a PNG when talking to the spice server.
    auto is_denlab_image = data_and_type.mime_type == "image/x-denlabos" && data_type == ClipboardDataType::PNG;
    if (!is_denlab_image && requested_mime_type.to_byte_string() != data_and_type.mime_type) {
    ...
    // If the mime type is `image/x-denlabos`, we need to encode the image that's on the clipboard as a PNG.
