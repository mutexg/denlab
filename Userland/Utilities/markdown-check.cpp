 ...
 * $ ninja -C Build/lagom
 * $ export DENLAB_SOURCE_DIR=/path/to/denlab
...
private:
    MarkdownLinkage(bool verbose)
        : m_verbose(verbose)
    {
        auto const* source_directory = getenv("DENLAB_SOURCE_DIR");
        if (source_directory != nullptr) {
            m_denlab_source_directory = source_directory;
        } else {
            warnln("The environment variable DENLAB_SOURCE_DIR was not found. Link checking inside DenLab's filesystem will fail.");
        }
    }
    ...
    ByteString m_denlab_source_directory;
};
...
RecursionDecision MarkdownLinkage::visit(Markdown::Text::LinkNode const& link_node)
{
    ...
    if (url.is_valid()) {
        ...
        if (url.scheme() == "help") {
            ...
            auto file = ByteString::formatted("{}/Base/usr/share/man/man{}.md", m_denlab_source_directory, URL::percent_decode(url.serialize_path()).substring(1));
        ...
        if (url.scheme() == "file" || url.scheme() == "launch") {
            ...
            if (file_path.starts_with("/res/icons/"sv)) {
                auto file = ByteString::formatted("{}/Base{}", m_denlab_source_directory, file_path);
                m_file_links.append({ file, ByteString(), StringCollector::from(*link_node.text) });
